...
def test_radix_sort_can_be_imported():
    """Test import works okay."""
...
@pytest.mark.parametrize("num_list, result", SUPER_TEST)
def test_insertion_sort_returns_correct_list(num_list, result):
    """Test insertion sort returns a properly sorted list."""
    from insertion_sort import insertion_sort
    assert insertion_sort(num_list) == result


@pytest.mark.parametrize("num_list, result", SUPER_TEST)
def test_merge_sort_returns_correct_list(num_list, result):
    """Test merge sort returns a properly sorted list."""
    from merge_sort import merge_sort
    assert merge_sort(num_list) == result
...
def test_radix_sort_raises_error_with_string():
    """Test input error is raised if string is used."""
    from radix_sort import radix_sort
    with pytest.raises(TypeError):
        radix_sort('sort this')
